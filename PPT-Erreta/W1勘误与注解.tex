...
\pdfpage{09}
\pdfpage{10}
\begin{frame}{\refpage{10}：关于条件（3）（4)可换的证明}
...
\begin{frame}{\refpage{25}：前两行结论的推导}
...
\begin{frame}{注第\refpageN{29}-\refpageN{30}页：无穷小生成元的理解及与常见文献中定义的对应}
	无穷小生成元，顾名思义，（不太严谨地）可以理解为与有限群的生成元在李群中的“对应”，即其他群元素可以用这个（些）元素不断地自乘得到。但由于李群是连续的，这样的“生成元”必然是无限小的，而我们会希望和有限的量打交道，所以我们定义这样的“无穷小生成元”时要除掉它“无穷小的程度”，也就是
	\begin{equation*}
	\text{无穷小生成元}=\frac{\text{无穷小群元}}{\text{该群元对应的无穷小群参数}}
	\end{equation*}
	课件中定义的无穷小生成元就是这样的：$\chi_\sigma(X)\delta\alpha^\sigma$给出底空间中一个函数经过一个$\delta\alpha^\sigma$对应的无穷小群操作后的变化量，即它表征了这个无穷小群操作，于是群参数前的系数就是我们要的无穷小生成元。
\end{frame}

\begin{frame}{注第\refpageN{29}-\refpageN{30}页：无穷小生成元的理解及与常见文献中定义的对应}
	
	“微分算子”这一形式在把等式写完整之后意义会更明晰。无穷小变化表示为$\chi_sigma(X)\delta\alpha^\sigma$，于是变化后的函数（等于原来的函数加上变化量）：
	\begin{equation*}
	(1+\chi_\sigma(X)\delta\alpha^\sigma)F(X^i)=(1+\delta\alpha^\sigma U^i_\sigma\pp{}{X^i})F(X^i)=F(X^i+U^i_\sigma\delta\alpha^\sigma)
	\end{equation*}
	这样可以明显地看出它表征一个无穷小平移操作。
	事实上，$\pp{}{X^i}$可以视为一组（协变）基矢\footnote{关于这一描述详见梁灿彬第二章，或其他微分几何教材}，这样$U^i_\sigma\pp{}{X^i}$可以视为一个矢量在这组基矢下的展开式，亦即$\chi_\sigma(X)$是底空间上的矢量场。
	
	并且，我们知道，每一个李变换群的元素$\gamma$都是一个底空间上的变换，那么它也就可以诱导一个底空间上矢量场的变换。具体来说，记变换函数为$Y^j(X^i)$，即原来底空间中坐标为$X^i$的点变换后被映射到了$Y^j$。那么原来在点$X^i$处的矢量$A^i(X)\pp{}{X^i}$，在映射并在新的坐标下展开后变成$A^i(X)\pp{Y^j}{X^i}\pp{}{Y^j}$。\footnote{这一过程严谨表述为一个推前映射，见梁灿彬第四章}
\end{frame}
...
\begin{frame}{注第\refpageN{29}-\refpageN{30}页：无穷小生成元的理解及与常见文献中定义的对应}
	...
	但我们知道
	\begin{equation*}
	A^i(X)=U^i_\sigma(X)=\ppat{f^i(\beta,X)}{\beta^\sigma}{\beta=0}
	\end{equation*}
	特别地有
	\begin{equation*}
	A^i(X)\pp{Y^j}{X^i}=\pp{Y^j}{X^i}\ppat{f^i(\beta,X)}{\beta^\sigma}{\beta=0}
	\end{equation*}
	由于合成函数$f^i$给出的结果其实就是$X^i$，并且是在$\beta=0$处取值，右边两项可以用链式法则合并：
	\begin{equation*}
	A^i(X)\pp{Y^j}{X^i}=\ppat{Y^j(f(\beta,X))}{\beta^\sigma}{\beta=0}
	\end{equation*}
	而
	\begin{equation*}
	A^i(Y)=\ppat{f^i(\beta,Y)}{\beta^\sigma}{\beta=0}=\ppat{f^i(\varphi(\beta,\gamma),X)}{\beta^\sigma}{\beta=0}
	\end{equation*}
	
	到这里我们发现，$A^i(X)\pp{Y^j}{X^i}\neq A^i(y)$。这说明这个矢量场并不“协变”（严格来说叫“左不变”）。
	
\end{frame}

\begin{frame}{注第\refpageN{29}-\refpageN{30}页：无穷小生成元的理解及与常见文献中定义的对应}
	但一般的文献（如梁灿彬、Cohn）中都是要求这个矢量场左不变的。这一要求的逻辑是：由于李群中每一个元素$g$都可以视为$ge$，从而$g$的邻域可以视为单位元$e$的邻域经过映射$L_g:x\mapsto gx$（这是一个微分和拓扑双重意义下的同胚，简单来说可以认为这个映射“保一切性质”）变换而得，李群中任意一点附近的性质都应该和单位元邻域相同。因此，无穷小生成元这个向量场应该是协变的，这样场中每一点事实上都是单位元附近的向量场映射而得。如果这个矢量场不是左不变的，那么可能出现问题，比如说无穷小生成元之间的关系变得依赖于坐标的选取。
\end{frame}

\begin{frame}{注第\refpageN{29}-\refpageN{30}页：无穷小生成元的理解及与常见文献中定义的对应}
	如果采取“左不变”的定义，那么之前定义的$f,U,V$都需要重写。$f$应该重写为$f(X,\beta)$，满足$f(X,\varphi(\alpha,\beta))=f(f(X,\alpha),\beta)$，亦即李变换群在底空间上是一个右作用。相应地，$U$改写为$U^i_\sigma(X)=\ppat{f^i(X,\beta)}{\beta^\sigma}{\beta=0}$，$V$改写为$V^\mu_\sigma(\alpha)=\ppat{\varphi^\mu(\alpha,\beta)}{\beta^\sigma}{\beta=0}$。
	
	这样的话，矢量场就是协变的了：
	\begin{equation*}
	A^i(X)\pp{Y^j}{X^i}=\ppat{Y^j(f(X,\beta))}{\beta^\sigma}{\beta=0}
	\end{equation*}
