...
\begin{frame}{\refpage{10}}
	此处默认了当$\alpha+\beta$是根的时候$N_{\alpha\beta}\neq 0$，事实上上面只说明了这个命题的否命题成立，无法直接说明这个命题本身成立，而下面的证明是用到了这个命题的。这是需要单独证明的，这里先默认它成立。%这个命题可以用课件最后的习题2的结论来证明。习题2的证明要用到第\refpageN{16}-\refpageN{18}页的定理，注意那个定理并没有用到“$\alpha+\beta$是根则$N_{\alpha,\beta}\neq 0$”的结论\footnote{因为即使在中间过程中}，只用到了“$\alpha+\beta$不是根则$N_{\alpha,\beta}=0$”。
	%为了不陷入循环论证，可将\refpageN{16}-\refpageN{18}页的定理重新表述为：一定存在$p,q$使得$N_{\alpha,\mu+p\alpha}=N_{-\alpha,\mu-q\alpha}=0$\footnote{这里并不用假设$N=0$之后外面的就不是根，但我们知道如果$N\neq 0$的话那么外面的一定是根，如果没有$N=0$截断就可以构造出无限个根，从而矛盾。}，在此基础上就可以推出习题2的结论，那么如果$N_{\alpha,\beta}=0$
\end{frame}
...
\begin{frame}{\refpage{14}：关于$E_\alpha=E_{-\alpha}^\dagger$的进一步说明}
	...
	\textbf{引理1：}\footnote{Varadarajan P285} 若$\alpha+\beta$是根，则有$N_{\alpha,\beta}=N_{-\alpha-\beta,\alpha}$
	
	\textbf{证明：}记$\gamma=-\alpha-\beta$，由雅可比不等式：
	\begin{equation*}
	...
	\end{equation*}
	$\alpha$和$\beta$亦线性独立，从而得到$N_{\alpha,\beta}=N_{\gamma,\alpha}=N_{-\alpha-\beta,\alpha}$\qedsymbol
...
\begin{frame}{\refpage{18}}
	...
	关于$(a,a)\neq 0$的证明：如果某个根$a$满足$(a,a)=0$，由上推导可以知道再取任意其他根$b$都会有$(a,b)=0$。我们知道$[E_\alpha,E_{-\alpha}]=a^iH_i$，其一定非零，否则$a^i=g^{ij}a_i=0$意味着$g^{ij}$退化，这与它可逆（由定义）矛盾。于是我们知道对于任意的$\beta$有$[[E_\alpha,E_{-\alpha}],E_\beta]=a^ib_i=0$，而由于各$H_i$可交换知道$[[E_\alpha,E_{-\alpha}],H_i]=0$。这意味着非零向量$[E_\alpha,E_{-\alpha}]$与李代数的整组基全部对易，从而这一向量张成的一维线性空间构成一个李代数的可交换理想，这与半单纯性矛盾。
